...
from config import DB_PATH, DB_INIT
...
def get_context():
    ...
    logging.info(f"Retrieved context from DB: {context}")
    return context
...
def get_history(chat_id: int, count: int = -1):
    ...
    """
    try:
        logging.info(f"Retrieving history for {chat_id}")
        db = sqlite3.connect(DB_PATH)
        if count == -1:
            res = db.execute(
        ...
        db.close()
        logging.info(f"Retrieved history: {out}")
    except Exception as ex:
       logging.info(f"Could not retrievev history for id {chat_id}")
       out = []
